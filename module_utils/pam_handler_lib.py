    ...
    if os.path.isfile(respamfile['filename']):
        ...
        with open(respamfile['filename'],"r") as pamfilehandler:
            ...
            while (pamline and breakeloop==0):
                ...
                palmlinerecord=getparsedpamline(pamline)
...
def pamlinepresent(service_name,pam_line,atpos,pam_fact,logdic):
    resultcode={}
    resultcode['rc']=0
    resultcode['results']=[]
    resultcode['stdout']=''
    ...
    #line to find
    linerecord=getparsedpamline(pam_line)
    if filecontent['fileexists']==True:
        #Process file in memory
        palmlinerecord=getparsedpamline(pam_line)
        entriefound=False
        entriefoundat=0
        filepos=0
...
def pamlinepresentatend(service_name,pam_line,pam_fact,logdic):
    resultcode={}
    filecontent=getfile(service_name,pam_fact)
    #print(filecontent)
    atpos=len(filecontent['filecontent'])
    resultcode=pamlinepresent(service_name,pam_line,atpos,pam_fact,logdic)
    return resultcode

def pamlinepresentafterlinerecord(service_name,pam_line,linereferencerecord,pam_fact,logdic):
    resultcode={}
    resultcode['rc']=2
    resultcode['results']=[]
    resultcode['stdout']=''
    ...
    if filecontent['fileexists']==True:
        ...
        if entriefound==True:
            resultcode=pamlinepresent(service_name,pam_line,entriefoundat+1,pam_fact,logdic)
        else:
            resultcode['rc']=4
...
def pamlinepresentafterline(service_name,pam_line,pam_line_after,pam_fact,logdic):
    resultcode={}
    print(pam_line_after)
    linereferencerecord=getparsedpamline(pam_line_after)
    resultcode=pamlinepresentafterlinerecord(service_name,pam_line,linereferencerecord,pam_fact,logdic)
    return resultcode
