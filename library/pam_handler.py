        ...
        if pam_process==1:
            ...
            if pam_state == 'report':
                ...
                if pam_state == 'present':                    
                    if pam_first== True:
                        rc=pamlinepresent(pam_service_name,pam_line,0,pam_fact,logdic)
                    else:
                        rc=pamlinepresentatend(pam_service_name,pam_line,pam_fact,logdic)
                ...
                if pam_state == 'presentafter':
                    ...
                    rc=pamlinepresentafterline(pam_service_name,pam_line,pam_line_reference,pam_fact,logdic)
                    if rc['rc'] >= 0 and rc['rc'] <= 2:
                        result['changed'] = True
                        rcsave=pamsavefile(pam_service_name,pam_fact,logdic,pam_backup)
                        if rcsave['rc'] == 0:
